...
def train_pokemon(backdoored_encoder, clean_encoder, data_loader, train_optimizer, args):
    backdoored_encoder.train()
    ...
    for img_clean, img_backdoor, img_clean_2, img_backdoor_2 in train_bar:
        img_clean = img_clean.cuda(non_blocking=True)
        ...
        if args.backdoor_mode == 'dittoencoder':
            img_clean_2 = img_clean_2.cuda(non_blocking=True)
            with torch.no_grad():
                clean_feature_raw_2 = clean_encoder(img_clean_2)
                clean_feature_raw_2 = F.normalize(clean_feature_raw_2, dim=-1)
            
            feature_raw_2 = backdoored_encoder(img_clean_2)
            feature_raw_2 = F.normalize(feature_raw_2, dim=-1)

            """
            object for ditto, x marker means used in loss
            [ ] f'(A*) == f'(A)
            [x] f'(A*) == f'(B)
            [x] f'(A) == f(A)
            [x] f'(B) == f(B)
            """
            loss_0 = - torch.sum(feature_backdoor * feature_raw_2, dim=-1).mean()
            loss_1 = torch.tensor(0)
            loss_2 = - torch.sum(feature_raw * clean_feature_raw, dim=-1).mean() * 0.5 \
                     - torch.sum(feature_raw_2 * clean_feature_raw_2, dim=-1).mean() * 0.5
            # """
            # ditto v2
            # [x] f'(A*) != f'(A)
            # [x] f'(A*) == f'(B)
            ...
            # [x] f'(B) == f(B)
            # """
            # loss_0 = torch.sum(feature_backdoor * feature_raw, dim=-1).mean() \
            #          - torch.sum(feature_backdoor * feature_raw_2, dim=-1).mean()
            # loss_1 = torch.tensor(0)
            # loss_2 = - torch.sum(feature_raw * clean_feature_raw, dim=-1).mean() \
            #          - torch.sum(feature_raw_2 * clean_feature_raw_2, dim=-1).mean()
            # """
            # ditto v3
            # [x] f'(A*) != f'(A)
            # [x] f'(B*) != f'(B)
            ...
            # [x] f'(B) == f(B)
            # """
            # img_backdoor_2 = img_backdoor_2.cuda(non_blocking=True)
            # feature_backdoor_2 = backdoored_encoder(img_backdoor_2)
            # feature_backdoor_2 = F.normalize(feature_backdoor_2, dim=-1)
            # loss_0 =   torch.sum(feature_backdoor * feature_raw, dim=-1).mean() \
            #          + torch.sum(feature_backdoor_2 * feature_raw_2, dim=-1).mean() \
            #          - torch.sum(feature_backdoor * feature_raw_2, dim=-1).mean() \
            #          - torch.sum(feature_backdoor_2 * feature_raw, dim=-1).mean() \
            #          + torch.sum(feature_backdoor * feature_backdoor_2, dim=-1).mean()   
            # loss_1 = torch.tensor(0)
            # loss_2 = - torch.sum(feature_raw * clean_feature_raw, dim=-1).mean() \
            #          - torch.sum(feature_raw_2 * clean_feature_raw_2, dim=-1).mean()
        elif args.backdoor_mode == 'mewencoder':
            """
            object for mew, x marker means used in loss
            [x] f'(A*) != f'(A)
            [ ] f'(A*) != f'(B)
            [x] f'(A) == f(A)
            [ ] f'(B) == f(B)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.encoder_usage_info == 'cifar10' or args.encoder_usage_info == 'stl10':
            if args.backdoor_mode == 'badencoder':
                ...
                train_loss = train_pokemon(model.f, clean_model.f, train_loader, optimizer, args)
